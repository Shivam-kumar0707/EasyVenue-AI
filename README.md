...
- **Code Quality**: Structured into small, single-purpose React hooks (`useLiveCrowdData.js`, `useIncidents.js`) and AI functions. Consistent formatting is enforced with Prettier and ESLint, and zero linter errors were reported by oxlint.
...
- **Efficiency**: API calls are heavily optimized. Anomaly detection only prompts the Groq API when a verified >30% crowd spike occurs (not on every tick). Summarization checks for incidents first, short-circuiting the AI call entirely if operations have been normal for the last hour.
- **Testing**: A full suite of 14 tests is implemented in Vitest (under `tests/`), testing input validation edge cases (null, empty, whitespace, and 500+ length limits), anomaly threshold logic (checks that spikes under 30% do not trigger warnings, while >30% spikes do), and classifyIncident's API error and JSON-parsing fallback logic.
